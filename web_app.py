...
# --- SESSION STATE SETUP ---
# This keeps data in memory while the user clicks around the web page
...
def main_app():
    ...
    # ==========================================
    # ADMIN ONLY VIEW: Can build the baseline model
    # ==========================================
        ...
        if base_file:
            ...
            if st.button("Run MLR Baseline Analysis", type="primary"):
                if target_y and selected_x_vars:
                    ...
                    # Save model to session state so Users can use it
    ...
    # ==========================================
    # ALL USERS VIEW: Can calculate savings
    # ==========================================
    st.markdown("---")
    st.header("Step 2: Reporting Period Analysis")
    
    if st.session_state['model_data'] is None:
        st.info("⚠️ Waiting for Admin to setup the Baseline Model in Step 1.")
    else:
